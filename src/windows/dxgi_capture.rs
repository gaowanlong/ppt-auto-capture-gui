...
use windows::Win32::Graphics::Direct3D11::{
    ID3D11Device, ID3D11DeviceContext, D3D11CreateDevice, D3D11_SDK_VERSION,
};
...
impl DxgiCapturer {
    ...
    pub fn initialize(&mut self, monitor: &MonitorInfo) -> Result<()> {
        ...
        unsafe {
            D3D11CreateDevice(None, D3D_DRIVER_TYPE_HARDWARE, None, 0u32,
        ...
        let device = dev.context("device")?; let _context = ctx.context("context")?;
        ...
        self.device = Some(device); self.duplication = Some(duplication);
        self.monitor_info = Some(monitor.clone()); self.frame_index = 0; info!("DXGI init OK");
        Ok(())
    }
    pub fn capture_frame(&mut self, timeout_ms: u32) -> Result<Option<Frame>> {
        ...
        match unsafe { dup.AcquireNextFrame(timeout_ms, &mut fi, &mut resource) } {
            ...
            Err(e) => { let hr = e.code();
                ...
                if hr == DXGI_ERROR_ACCESS_LOST { return Err(anyhow::anyhow!("DXGI lost")); }
        ...
        unsafe { surface.Map(&mut mapped, 1u32).ok().context("Map")?; }
        let (w, h) = (sd.Width, sd.Height);
        let data = unsafe { std::slice::from_raw_parts(mapped.pBits as *const u8, (mapped.Pitch as u32 * h) as usize) }.to_vec();
        unsafe { surface.Unmap().ok(); }
        let _ = unsafe { dup.ReleaseFrame().ok() };
        self.frame_index += 1;
        Ok(Some(Frame::new(data, w, h, mapped.Pitch as u32, self.frame_index, now_ms())))
    }
...
fn find_output(adapter: &IDXGIAdapter, mon: &MonitorInfo) -> Result<IDXGIOutput> {
    ...
    loop {
        if let Ok(o)=unsafe{adapter.EnumOutputs(i)} {
            if let Ok(d)=unsafe{o.GetDesc()} { let rc=d.DesktopCoordinates;
