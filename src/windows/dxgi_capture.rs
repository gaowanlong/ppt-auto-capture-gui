...
use windows::Win32::Graphics::Dxgi::{
    ...
    DXGI_MAPPED_RECT, DXGI_MAP_FLAGS, DXGI_ERROR_WAIT_TIMEOUT, DXGI_ERROR_ACCESS_LOST,
};
...
impl DxgiCapturer {
    ...
    pub fn initialize(&mut self, monitor: &MonitorInfo) -> Result<()> {
        ...
        self.device = Some(device); self.context = Some(context);
        self.duplication = Some(duplication);
        self.monitor_info = Some(monitor.clone());
        self.frame_index = 0; info!("DXGI init OK");
        Ok(())
    }
    ...
    pub fn capture_frame(&mut self, timeout_ms: u32) -> Result<Option<Frame>> {
        ...
        unsafe { surface.Map(&mut mapped, DXGI_MAP_FLAGS(1)).ok().context("Map")?; }
        let w = sd.Width; let h = sd.Height;
        let data = unsafe {
            std::slice::from_raw_parts(mapped.pBits as *const u8, (mapped.Pitch as u32 * h) as usize)
        }.to_vec();
